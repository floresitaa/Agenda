                        ...
                        @if ($tarea->archivo_url)
                            <div class="mt-2">
                                @if (Str::contains($tarea->archivo_url, ['.jpg', '.jpeg', '.png', '.gif', '.bmp']))
                                    <img src="{{ asset('archivos/tareas/' . basename($tarea->archivo_url)) }}" alt="Archivo" style="max-width: 200px; max-height: 200px;">
                                @elseif (Str::contains($tarea->archivo_url, ['.mp4', '.mov', '.avi']))
                                    <video controls style="max-width: 200px; max-height: 200px;">
                                        <source src="{{ asset('archivos/tareas/' . basename($tarea->archivo_url)) }}" type="video/mp4">
                                        Your browser does not support the video tag.
                                    </video>
                                @else
                                    <a href="{{ asset('archivos/tareas/' . basename($tarea->archivo_url)) }}" target="_blank" class="text-indigo-600 hover:text-indigo-900">{{ basename($tarea->archivo_url) }}</a>
                                @endif
                            </div>
                        @endif
                        @error('archivo_url')
